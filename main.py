...
ADS_CID = os.environ.get("ADS_CUSTOMER_ID", "5802443211")
...
def structure(client, region, raw, services):
    ...
    prompt = (
        ...
        "Donne de 6 a 10 clusters selon l'etendue REELLE des services du client : ne "
        "force pas le nombre, n'invente aucun cluster mince ou redondant. Reponds "
        "UNIQUEMENT en JSON : " + SCHEMA
    )
...
def refine_result(prev, instruction, region):
    prompt = (
        ...
        "complet (garde 6 a 10 clusters, conserve note_donnees et sources_recommandees). "
